...
public static class FluentIconsMcpApplication
{
    ...
    {
        var rootCommand = new RootCommand("Runs the Fluent Icons MCP server or manages provider MCP registration.");
    ...
    private static Command CreateMcpConfigurationCommand(string name, string description, bool isInstall, IProviderCommandRunner providerCommandRunner, TextWriter outputWriter, TextWriter errorWriter)
    {
        ...
        var nameOption = new Option<string>("--name")
        {
            ...
            DefaultValueFactory = _ => FluentIconsMcpConstants.DefaultServerName,
        };
        ...
        command.SetAction((parseResult, cancellationToken) => ExecuteMcpConfigurationCommandAsync(isInstall, parseResult.GetValue(providerOption), parseResult.GetValue(configOption), parseResult.GetValue(nameOption) ?? FluentIconsMcpConstants.DefaultServerName, providerCommandRunner, outputWriter, errorWriter, cancellationToken));
    ...
    private static async Task<int> ExecuteMcpConfigurationCommandAsync(bool isInstall, string? providerValue, string? configPath, string serverName, IProviderCommandRunner providerCommandRunner, TextWriter outputWriter, TextWriter errorWriter, CancellationToken cancellationToken)
    {
        ...
        var changed = isInstall ? configurationManager.Install(configPath!, serverName, FluentIconsMcpConstants.ToolCommandName) : configurationManager.Remove(configPath!, serverName);
    ...
    {
        var providerCommand = isInstall ? ProviderCommandBuilder.CreateInstallCommand(provider, serverName, FluentIconsMcpConstants.ToolCommandName) : ProviderCommandBuilder.CreateRemoveCommand(provider, serverName);
    ...
    private static async Task<int> RunMcpServerAsync(CancellationToken cancellationToken)
    {
        ...
        builder.Services.AddSingleton<FluentIconsDataService>();
        builder.Services.AddHostedService<FluentIconsBootstrapService>();
